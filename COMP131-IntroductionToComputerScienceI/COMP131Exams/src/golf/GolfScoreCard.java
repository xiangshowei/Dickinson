...
/**
 * A class representing a golf score card.  It will keep track
 * of the scores for a number of holes of golf.
 * 
 * @author Xiang Wei
 * @version 7/23/2016
 */
public class GolfScoreCard {
	...
	/**
	 * Construct a new GolfScoreCard.  For our purposes, we'll assume
	 * that the holes are numbered starting with 0.
	 * 
	...
	/**
	 * Get the player's score for the specified hole. If the hole
	 * is not valid then this method returns -1.
	 * 
	...
	/**
	 * Determine if this player is good enough to play with professional golfers.
	 * In order to be good enough to play with Tiger, the player's may have
	 * no score larger than a 5 and can have at most 3 scores of 5.
	 * 
	 * @return true if the player is good enough to play with Tiger and
	 * false otherwise.
	 */
	...
	/**
	 * Get the hole on which the player had his/her highest
	 * score.  If there is a tie for the highest score, then the hole
	 * with the lower number should be returned. 
	 * 
	...
	/**
	 * One method of leveling the playing field for players of different ability
	 * is to use a handicap system.  Each player is assigned a handicap, with higher
	 * handicaps going to weaker players. A player's score card can then be modified
	 * based on their handicap after a round to make weaker players better relative to 
	 * stronger players.
	 * 
	 * One technique for applying handicap works as follows.  The player's handicap
	 * represents a number of strokes to be removed from a player's score. Each stroke
	 * of the handicap reduces the player's highest score by one stroke.  For example:
	 * if a player has the scores 4, 7, 3, 8, 4 and a handicap of 4 then the scores
	 * would become 4, 5, 3, 6, 4.  First the 8 is reduced to a 7, then one of the 7's
	 * is reduced to a 6, then the remaining 7 is reduced to a 6 and finally, one of the
	 * 6's is reduced to a 5.  When there are multiple instances of the maximum score, 
	 * any one of them may be reduced.
	 * 
	...
	/** 
	 * This method is for those who are not good at golf but have a need to 
	 * appear good at golf that exceeds the strength of their moral fiber.  
	 * It creates and returns a new GolfScoreCard object with every score reduced 
	 * by the specified number of strokes.  The one caveat is that if reducing a score
	 * by the specified number of strokes would make it 3 or less, then that score is 
	 * not changed.  For example, if the player has the scores 5, 6, 9, 2, 7 and they 
